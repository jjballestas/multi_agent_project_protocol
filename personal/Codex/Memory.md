...
- Commit `4cfd1b03` binds the rollback ledger assertion in production `main()` to the existing
  permanent negative through semantic AST inspection. The criterion finds the call to
  `ledger_preservation_holds` inside an assertion and does not enumerate reason names or source
  forms.
- The run-derived balance is `baseline=1 coordinate=1 order=1 format=1 deleted=0`; changing line
  coordinates, definition order, or formatting preserves the criterion. Removing the complete
...
- Commit `8ab9d575` replaces value-shape exemptions with validated-coordinate handling. Protocol
  identity fields and `file`/`path` coordinates remove only their protocol envelope; the same
  sensitive payload remains detected bare, in a path segment, and in an identity suffix.
- The permanent account-identifier negative now covers both loss and gain directions at the
  exemption coordinate. Its powered 16-value cross-coordinate population has 11 prior-engine
  positives, 16 current positives, 5 gains, and 0 losses; the coordinate-blind mutant loses 12.
...
- Delivery commit `03234d57` moves TASK-0336 to `in_review`, releases all three remediation-5
  claims, updates the self-contained handoff, and publishes the Arquitecto routing request for
  independent Analista B3 re-review. Runtime drift is false through sequence 8297. Codex has not
  reviewed or ratified the remediation.
...
- Commit `45bed5d` removes the self-bumped exec-lease heartbeat from
  ...
  for lease ownership and stale-lock behavior, but cannot keep a frozen exec alive.
- A new production-freshness regression runs with `ProgressFreshSeconds=15`,
  freezes both run-log and ledger output, and requires `reason=no_progress`
  before the hard cap with complete tree cleanup. The test also rejects any
