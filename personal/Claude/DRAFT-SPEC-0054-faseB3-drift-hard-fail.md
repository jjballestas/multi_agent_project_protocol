...
> DRAFT en personal/Claude/ (preparado mientras Codex implementa B.2, DECISION-0020 #1). Tercera rebanada de
> la Fase B (SPEC-0039 sec.4). NO promover a ready hasta que B.2 (TASK-0067) este done: B.3 se monta sobre la
> materializacion de B.2 ("runtime = escritor habitual" es la condicion que activa el hard-fail). El diseño
> solo depende de B.1 (done). Aditivo, triple-gated, off=byte-equivalente. Cambio incompatible => blocked + DECISION.
...
Encolar como TASK-0068 SOLO cuando TASK-0067 (B.2) este `done` (promover de a una, DECISION-0020 #7).
Tras B.3, queda B.4 (migrar/prohibir edicion manual) que probablemente requiera DECISION nueva + evaluacion
SemVer (posible MAJOR) y aprobacion humana.
