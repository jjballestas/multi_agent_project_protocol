...
# TASK-0068 - Fase B.3: drift del estado de protocolo como hard-fail (DRAFT, no promovida)

> DRAFT en personal/Claude/. Promover a ready SOLO cuando TASK-0067 (B.2) este done (DECISION-0020 #7).
> Se monta sobre la materializacion de B.2. Ver SPEC-0054.

## Contexto

B.1 (done) dio replay/drift read-only (drift=WARNING). B.2 (en curso) materializa el estado desde el log
...
Fase B: `hot *.json == materializa(replay(log))` como hard-gate.
...
Tercera de 4 rebanadas de Fase B (B.1 done -> B.2 en curso -> **B.3 hard-fail** -> B.4 migrar/prohibir
edicion manual, posible DECISION/MAJOR + aprobacion humana).
