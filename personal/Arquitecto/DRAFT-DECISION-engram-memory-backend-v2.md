---
decision_id: DECISION-00XX (DRAFT v2 - id final al promover)
title: Engram como backend de memoria/recall (capability OFF by default) - frontera ledger/memoria, namespacing por agente, single-writer preservado, activacion Tier 1 por gate atestado (estructural por diseño, valido en runtime-tier) y no-interferencia con el dataset del TFM
...
# DECISION-00XX (DRAFT v3) - Engram como backend de memoria/recall

> DRAFT v3 del Arquitecto. v2 cerraba los bloqueantes del Analista con DISEÑO solido pero la verificacion
...
> "estructural por diseño" no equivale a "cerrado". Esta DECISION es promovible **OFF** como "capacidad
...
- **[ESTRUCTURAL]** El intent `engram_observation` es un **no-op puro** sobre el state materializado:
  ...
  para este tipo; lo hacen cumplir el hard-gate B.3 de drift y el test de replay byte-identico (PATCH v2 §3).
...
- **Tier 1 (DIFERIDO en su ACTIVACION, ver clausula 5):** memoria **compartida** entre agentes, enrutada
  ...
  por diseño, valido en runtime-tier; ver clausula 5 y la Matriz de estado).
...
(`runtime/submit_intent.py:662`), unico chokepoint que cubre el camino single y el `--intents` (PATCH v2 §2).
Cuatro garantias:
...
- **(c) Cero PROSA en el ledger [ESTRUCTURAL-PENDIENTE-IMPL+TESTS]:** el evento `engram_observation` lleva
  ...
  valido AUN pasaba. v3 los regimenta por enum/regex (PATCH §2.2/§2.4), de modo que NINGUN campo admite prosa
  por valor. SOLO ENTONCES "cero-PROSA en el evento" es estructural; mientras el PATCH no este merged con su
  test `test_engram_rejects_prose_in_admitted_fields` verde, es estructural POR DISEÑO, PENDIENTE. El cuerpo
...
`enabled/materialize/enforce/authoritative` y bajo su misma guardia de configuracion. El diseño es
estructuralmente solido y los anclas verifican, PERO no hay codigo merged ni tests ejecutados: lo de abajo
es **estructural POR DISEÑO, PENDIENTE de implementacion+tests verdes** (ver Matriz de estado y seccion H).
TRES garantias, con su etiqueta y precondicion:

- **5.1 GATE ENFORCED [ESTRUCTURAL-PENDIENTE-IMPL+TESTS; el diseño verifica, el codigo no esta merged].**
  ...
  los tests negativos del PATCH §5 son a ESCRIBIR; `grep engram_* runtime/*.py` = 0 hits. Sube a ESTRUCTURAL
  ...
  disciplinario.
- **5.2 FLIP ATESTADO, NO EDITABLE A MANO [ESTRUCTURAL-SI-PRECONDICION(chain_enabled=true); diseño verificado, impl pendiente].** Como `compute_genesis_prev_hash`
  ...
  diseño; la prueba estructural (`test_config_flip_breaks_chain_without_regenesis`, PATCH I10) esta pendiente
  de escribir.
- **5.3 PRECONDICION 'DATASET SELLADO' [ESTRUCTURAL-SI-PRECONDICION(adoption_tier=='runtime'); el JUICIO del corte es DISCIPLINARIO].** Encender
  ...
  **INERTE en coordination-tier**. Por eso `engram_capability_enabled` debe exigir tambien
  `adoption_tier=='runtime'` (PATCH §2.3): asi la capacidad solo enciende donde el seal tiene dientes, y el
...
- El protocolo gana una via atestada, single-writer y **con namespacing de autor enforced en el borde** para
  ...
  diseño). **Honestidad:** el namespacing es anti-typo estructural; contra impersonacion requiere
  `actor_auth_enforce` atestado (hoy en override gitignored -> disciplinario), declarado como precondicion.
- La activacion de Tier 1 deja de ser una promesa documental: el diseño la ata a un gate de configuracion
  ...
  dataset (eso es operador, DISCIPLINARIO); y el mecanismo es diseño, no codigo merged.
...
- **HONESTIDAD (estado de cierre, v3):** NADA se declara cerrado. La Matriz de estado honesta v3 (al final)
  da la etiqueta exacta por bloqueante. En resumen: A/B/E/H tienen **diseño estructural verificado pero SIN
...
de la Matriz se etiquete "cerrado": A/B/E/H son estructural POR DISEÑO y suben de nivel solo al mergear con
...
- **ENG-OUTBOX / ENG-BRIDGE (bloqueante C, condicion previa a Tier 1):** `runtime/engram_bridge.py`
  ...
  `content_sha256`. **HUECO DE DISEÑO ABIERTO en C:** el evento `engram_observation` NO lleva puntero al
  ...
  o declarar el direccionamiento del cuerpo como punto de diseño ABIERTO a resolver antes de Tier 1.
...
- **`title` libre con blacklist de cuerpo (v1, I4).** Descartada: disciplinario disfrazado de estructural;
  ...
  estructural por diseño (pendiente de impl+tests).
...
> (diseño verificado por anclas, sin codigo merged ni tests ejecutados) | **ESTRUCTURAL-SI-PRECONDICION(<cual>)** |
...
|---|---|---|---|
| **A-activacion** (gate de activacion de Tier 1) | **ESTRUCTURAL-SI-PRECONDICION(adoption_tier=='runtime')** por diseño, **PENDIENTE-IMPL+TESTS** | Mergear el gate en `validate_intent` + el guard `dataset_seal` en `event_state_config_error` + `engram_capability_enabled` exigiendo `adoption_tier=='runtime'`; tests I1/I9 + `test_engram_off_in_coordination_tier` verdes en clon limpio. En coordination-tier el guard es inerte (`protocol_replay.py:127`) -> alli baja a DISCIPLINARIO. | Decision 5.1-5.4; PATCH §2.3 (gate+tier), §3b (seal), §5 |
| **A-flip** (no editable a mano) | **ESTRUCTURAL-SI-PRECONDICION(chain_enabled=true)** por diseño, **PENDIENTE-IMPL+TESTS** | Test `test_config_flip_breaks_chain_without_regenesis` (I10) verde. Bajo `chain_enabled=false` baja a DISCIPLINARIO (declarado). Ancla `compute_genesis_prev_hash` (eventlog.py:185-189) verifica el diseño. | Decision 5.2; PATCH §0.1(2), I10 |
...
| **C-atomicidad** | **ABIERTO-DIFERIDO** | Construir ENG-OUTBOX/ENG-BRIDGE (consumidor idempotente + ACK fuera del ledger) + ENG-RECONCILE (healer). **Hueco de diseño abierto:** el evento no lleva `body_ref` al cuerpo cuyo sha256 atesta -> el bridge no puede localizarlo; anadir `body_ref` (slug de path) o declararlo punto abierto. El ledger atesta COMPROMISO, no existencia. | Decision 4-ter; PATCH §7, §8; tareas ENG-OUTBOX/ENG-BRIDGE, ENG-RECONCILE |
...
| **E-identidad** | **ESTRUCTURAL-PENDIENTE-IMPL+TESTS** (anti-typo) / **ESTRUCTURAL-SI-PRECONDICION(actor_auth_enforce atestado)** (anti-impersonacion) | El namespacing `memory_project=="map-"+binding_slug(actor_id)` en `validate_intent` es estructural por diseño (falta merge + test I7). PERO el binding ed25519 es no-op hoy: `actor_auth_enforce` vive en el override runtime gitignored (`eventlog.py:294-296`), no en el config atestado -> `--actor-id` spoofable. Promover `actor_auth_enforce` al plano atestado (ENG-ACTORAUTH-ATTEST) para que sea anti-impersonacion. | Decision 4(a)/(b), 4-bis; PATCH §2.5, I7; tarea ENG-ACTORAUTH-ATTEST |
| **H-parche** (suite + chokepoint unico + config) | **ESTRUCTURAL-PENDIENTE-IMPL+TESTS** | Enforcement en el chokepoint unico `validate_intent` (cubre single + `--intents`) y flag en el plano atestado: diseño verificado. Falta mergear y dejar verde toda la suite I1..I10 + espejo PowerShell del guard + scans de neutralidad/encoding en clon limpio. | Decision 5.1 + Frontera DECISION-vs-PATCH; PATCH §1, §5, §6, §7 |
