...
relates_to: [DECISION-0051, DECISION-0052, DECISION-0053, DECISION-0040, DECISION-0050, REQ-ZEUS-001]
phase: P2
---

# DECISION-0106 - Consola de prompt operador->agente desde el front (Alcance A / P1 de TASK-0178)

> PROPUESTA para ratificacion del operador. Gate de superficie de escritura (CLAUDE.md regla 2 / AGENTS.md s4).
> Codigo SOLO en Zeus-protocol (producto); core neutral. NO toca el config pinned (#4 epoca 1.14.0), ni
> INTENT_TYPES del runtime, ni genesis/keys.
...
4. **Atribucion honesta (accountability, patron 0052):** `author: Operador`. Si el front commitea el MSG, es
...
## Verificacion de la ratificacion (que gate la SPEC despues)
Builder server-side (forma estricta, prueba negativa de forma-ajena RECHAZADA), ASCII/PII, `operator-directive`,
`author:Operador`/`relayed_by`/`endorsement:none`, off-by-default (flag fuera del config), vista de hilo
read-only, #4 byte-identico (drift 0 necesario no suficiente), core neutral, npm test producto exit 0.
