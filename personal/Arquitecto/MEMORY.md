...
- **PROXIMO (operador decide):** (a) re-trabajar los drafts segun las 7 correcciones del Analista (allowlist title/topic_key, outbox+ack del bridge, importador/eliminar promesa de reconstruccion, enforcement actor->project, parche real+tests, mover ejemplos PII de dominio fuera del core) y re-someter a otra ronda del Analista (GO-CON-CONDICIONES), o (b) dejar Engram en Tier-0 local privado sin DECISION ni PATCH. Cron Analista sigue vivo (auto-exit por idle tras 7 rondas sin respuesta).
