...
> Ultima actualizacion: 2026-07-01 (sesion), HEAD b73aa90 (PUSHED), v1.14.0 (#4 enforce/auth ON; 5 pineados). FASE = **REQ-ZEUS-001 PRODUCTIZACION EN EJECUCION** (dataset SELLADO N=500, H1-H3 confirmadas -- eso NO se toca).

## >>> ESTADO VIGENTE 2026-07-01 -- REQ-ZEUS PRODUCTIZACION EN EJECUCION (coordinacion semi-auto) <<<
...
- **REQ-ZEUS-001 ADOPTADO en el HUB (una cadena atestada):** `DECISION-0077` adopcion. **D1-D5 = DECISION-0072..0076** (scope:product, relates_to [REQ-ZEUS-001, DECISION-0050], en Area_comun/decisions/ vivo NO templates). D1 tiered / D2 una-instancia / D3 backend-hibrido+guia-peones / D4 hermes-agent MIT vendorizar-permitido / D5 rebranding superficial. Canonicalizan las NOVA D1-D5. Producto en `D:\Agentes\Zeus\Zeus-Aegis`; instancia-plantilla NOVA (`D:/Agentes/Zeus/NOVA`).
- **NUMERACION (regla operador):** tareas en el HUB **TASK-02xx** (`TASK-\d{4}`, el validador solo acepta eso); agrupamiento producto = titulo `[REQ-ZEUS-001][WSx]` + `relates_to: REQ-ZEUS-001`. NO namespace TASK-ZEUS.
- **PIPELINE (2026-07-01 sesion nueva):** **DONE:** 0223, 0225, 0226, 0228 (WS5), **0224 (redactor, Codex done-flip commit d85e0be)**. **in_review:** 0222 (stats: NO-GO npm test EXIT124 -> ACTION-remediacion en open, Codex aun no reentrega) y **0227: 4o NO-GO del Analista (rem-3): npm test YA VERDE (559 EXIT0), pero guard estatico filtra firmas dinamicas (opts var, new Request, axios.request posicional). Operador GO -> DECISION-0079 (scope product) ACOTA el AC F1: guard cubre firmas ENUMERABLES literales, lo dinamico queda FUERA (garantia real = endpoint backend read-only + review). Ruteada rem-4 a Codex = RONDA FINAL (3 firmas nombradas), cierra contra AC acotado sin mas rondas.** LECCION: guard estatico read-only es arms-race indecidible; no relitigar rondas infinitas, acotar AC. rem-4 ENTREGADA (producto 534b95e, npm test clon limpio 559 EXIT0, cubre las 3 firmas) -> RUTEADA al Analista REVIEW-rem-4 vs AC acotado (commit 2a881e6). Espero GO -> cierre.
- **0222: NO-GO rem-1 del Analista (commit 3eea590): a68eb34 SI arregla el timeout focal de stats (1142ms, 500/500) pero full npm test clon limpio EXIT1 por ERR_IPC_CHANNEL_CLOSED. LECTURA: es FLAKE ambiental de teardown de workers vitest (el MISMO suite pasa en clon limpio de 0227 rem-4; a68eb34 ortogonal a IPC) + ~45 node huerfanos. Ruteada rem-2 a Codex (2a881e6): limpiar workers huerfanos + reconfirmar EXIT0 REPETIBLE + endurecer teardown. Se relaciona con 0235 exec-lease.** LECCION: ERR_IPC_CHANNEL_CLOSED = flake de runner, no defecto; pedir prueba de EXIT0 repetible, no una sola corrida.
- **BUG de coordinacion confirmado 2x hoy: el cron del peer EXEC_STARTa sobre el archivo en working-tree ANTES de que yo commitee -> clona HEAD sin el mensaje -> no-op de ~40s + marca seen (DROP). Paso con rem-4 (Codex) y con la review 0222 (Analista dropeo veredicto tras 24min). REMEDIO: limpiar su entrada en .protocol-tmp/<peer>_mailbox_cron/*.seen.json y re-disparar (funciona en el reintento). Motiva 0235.** NB: Codex commitea LOCAL y a veces NO pushea (0783730/d85e0be quedaron solo local); publico yo al pushear. **REDESPLIEGUE + REMEDIACION JAMS 2026-07-02 (HEAD ref 9293026):** **Crons REDESPLEGADOS** con el harness endurecido (0235): tear-down de los loops viejos (habia DOS instancias Codex = causa de contencion) + relanzamiento en modo NO-AUTO (el operador me paso a no-auto; en auto el launch se deniega). **STOP_JOB activo -> footgun OFF** (ya puedo escribir kill/deadline/parada en mensajes). **Registradas+promovidas+GO a Codex: TASK-0236** (INFRA, remediacion 0235, 4 fixes al harness: prompt POR-EXEC en runs/ no compartido + deadline-kill de ARBOL `taskkill /T` no un-pid + guard instancia-unica + enforcement lease huerfana) y **TASK-0237** (PRODUCTO Zeus-Aegis, PRIORIDAD: hang-proof del npm test = vitest run sin watch + testTimeout duro + teardown mata esbuild/node + CI=1). **CAUSA RAIZ de los jams confirmada por operador: el npm test del producto SE CUELGA en clon limpio (run 20260701T222832Z) y cada tarea WS lo corre.** **0229 DIFERIDO** (marque su GO seen) hasta que 0237 arregle el hang; su work ya esta en producto (980445c). **Orphan claim `CLAIM-20260702-Codex-TASK-0229` activo** (exec murio sin liberar; over-scopea events.jsonl -> uso claims LIGHT-scope para registrar sin chocar; se resolvera al re-EXEC de 0229 tras 0237). **MONITORES: 2 persistentes** -- entregas (`bi148oyz4`, HEAD-local) + **watchdog de salud de execs (`bnx2srvj6`): detecta cuelgue por lock-retenido + run-log CONGELADO >8min** (cierra el hueco "exec muerto no dispara nada"). **CHECKPOINT 02:48 (HEAD 37cbd5d) -- deploy 0236 + higiene + DEC-0080:** (a) **DEPLOY 0236 HECHO**: teardown de crons + relaunch con el harness de 5 fixes. **El detector de corte ahora es IGUALDAD EXACTA** (`($requested.Trim() -ceq "STOP_JOB")`), NO contains -> **el footgun MURIO: ya puedo MENCIONAR "STOP_JOB"/stop/parada en mensajes; solo un requested_action que SEA exactamente "STOP_JOB" detiene al agente.** (Footgun-2 descubierto hoy: el `-cmatch` viejo tumbo el cron de Codex porque GO-0236 MENCIONABA STOP_JOB.) (b) **0236 DONE** (Codex `08db237`). LECCION: el done-flip review_approved->done exige capability `implementer` -> **el Arquitecto NO puede hacer done-flips, solo Codex**; yo ratifico review_approved. (c) **DECISION-0080** registrada: politica A/B (barrido quirurgico execs colgados + baja graceful), GO sustantivo del Analista + guards normativos. (d) **Higiene mailbox**: los TX-hyg-2/3/4 estaban OBSOLETOS (ya archivados en el barrido de 60); archive los 12 consumidos actuales (open/=3: re-gate 0237, entrega-rem 0237, GO-0229 diferido). (e) **0237 en re-gate**: NO-GO (watchdog vendor no mataba el arbol) -> Codex remedio (producto ea3f52c: tree-kill sincrono + exit124 acotado) -> re-ruteado al Analista (le limpie el seen tras el redeploy). (f) **0229 BLOCKED** (re-habilitar tras 0237). LECCION drift: cada entrega/flip de Codex deja .md/index desalineado -> SIEMPRE verifico con CLON LIMPIO (--no-hardlinks) antes de confiar; el workspace vivo miente. Watchdog de salud (`bnx2srvj6`) cazo el cuelgue del Analista hoy (funciona).
**EN VUELO AHORA (01:32, HEAD cc1dac4):** 0237 in_progress (Codex, hang-proof npm test, PRIORIDAD); 0236 ready (cola Codex tras 0237); 0229 BLOCKED (work en producto 980445c, re-habilitar tras 0237); review A/B en Analista; 0233 proposed. Monitores: entregas `bi148oyz4` + watchdog salud `bnx2srvj6`. Prompt de arranque refrescado: SESSION_START_PROMPT_20260703.md (estado real post-redespliegue). Skills actualizadas: arquitecto-monitor-coordina (watchdog 2o monitor obligatorio) + arquitecto-cron-lifecycle (s.4b redespliegue + s.4c tree-kill/who_locks-self-match/seen-defer).
**HECHO (c5f2cef): TASK-0233 (WS7 e2e VM limpia) REGISTRADA proposed owner Analista (NO promovida, deps WS1-WS6); review A/B de la politica cron-zombie RUTEADA al Analista (GO/NO-GO sobre A barrido quirurgico + B baja graceful; C cerrada por 0235). Con GO del Analista -> redactar la DECISION correspondiente.** **DESTRABADO MANUAL DE HOY (receta): who_locks.py (Restart Manager) pinpoint holders del prompt -> taskkill /T cada uno -> rm lock/lease -> relanzar UNA instancia. Ojo: mis propios powershell de query se auto-matchean 'mailbox_cron.ps1' (falsos positivos); usar el LOG (frozen=down) como verdad.**
**CIERRE 2026-07-02 (HEAD ref 4327e40):** bloque **0222 0223 0224 0225 0226 0227 0228 0235 = DONE**. **Higiene mailbox HECHA: open/ VACIO** (60 archivados, 12 lotes de 5 en background; validate=0). **SIGUIENTE PASO PIPELINE: promover TASK-0229 [WS3]** (proposed->ready + GO Codex). Prompt de arranque nuevo: `SESSION_START_PROMPT_20260703.md`. SUPERADOS (nota al tope): `PLAN-cierre-dataset-lote-panel.md` + `REQZEUS-backlog-map.md` (numeracion vieja; vigente = PLAN-REQ-ZEUS-001-reconciliado + DECISION-0077). **DOS PENDIENTES DE GO operador (orden pedido higiene->redespliegue->skill; higiene ya): (2) REDESPLIEGUE harnesses (relanzar crons, permission-gated; activa STOP_JOB+self-heal+deadline, apaga footgun); (1) SKILL arquitecto-retro (destila lecciones->skills, con guardrails).** GOTCHA higiene: submit_intent re-replaya ~40s creciendo -> lotes de 5 en BACKGROUND, nunca foreground 2min (kill a mitad = half-apply; recuperar `git checkout HEAD -- state/mailbox` + `git clean` archived + validate=0).
**FIX MONITOR (leccion 2026-07-01, reprension operador "por que no te das cuenta si no te gatilleo"):** el monitor debe vigilar el **HEAD LOCAL**, NO `origin/main`. Los peers commitean al arbol COMPARTIDO pero **casi nunca pushean** -> un watcher de origin no ve las entregas locales y me hace depender de que el operador diga "revisa". El comando corregido (HEAD local + MSG `*-to-Arquitecto-*` nuevos, self-filter) esta en el skill arquitecto-monitor-coordina. Al reaccionar: fetch + coordino + **pusheo yo el commit local del peer** + re-armo.
**SESION TARDE 2026-07-01 (post-0222):** (a) **0222 DONE**. (b) **0227 = arms-race del guard F1**: 6 NO-GO del Analista, cada ronda una forma sintactica nueva (inline/shorthand/computed/const-tipado/clave-quoted). DEC-0079 acota + **AMENDMENT (operador GO tras 6 rondas) CIERRA la familia** en "claves literales method/url en toda forma (quoted+unquoted+computed literal)"; lo dinamico (dataflow/alias/variable) FUERA definitivo -> backend read-only. **rem-6 = ULTIMA ronda (commit 75cacd7): patron `["']?(method|url)["']?:`. NO habra rem-7.** Riesgo real siempre NULO (backend rechaza igual). (c) **STOP_JOB (directiva operador, commit affe347):** los harnesses de cron ahora se detienen SOLO con el token exacto `STOP_JOB` (`-cmatch "\bSTOP_JOB\b"`, solo summary/requested_action) -- mata la ambiguedad del regex viejo (que se auto-disparaba con "para" preposicion y con la tarea 0235 que contiene "stop marker"). Se activa al RELANZAR crons (diferido hasta GO de 0235). (d) **BUG recurrente: el exec de review del Analista (OpenAI Codex CLI gpt-5.5) MUERE ~10min en reviews Zeus-Aegis pesados** (no es deadline, ExecTimeout=3600s; muerte real del proceso). Deja lock huerfano; **el self-heal del harness solo limpia por deadline-vencido, NO por PID-muerto-antes** -> bloquea hasta 1h. Destrabo manual (rm lock) + re-disparo; el reintento suele completar. HALLAZGO para el gate de 0235: requisito #2 debe limpiar tambien en PID-muerto-pre-deadline. (e) **Codex sobre-scopea claims con el ledger** (events.jsonl/snapshot.json por horas) -> bloquea submit_intent de todos (el Analista no pudo emitir veredicto de 0235). Anomalia DECISION-0018; se libera al entregar. Al rutear ledger-ops, ventana libre = 0 claims activos.
**LANZADA por directiva operador: TASK-0235 (exec-lease hardening) promovida proposed->ready (seq 2880) + GO a Codex (footgun-safe, sin palabra-stop pese a que el spec es sobre kill/cron). Resuelve de raiz los zombies/locks huerfanos/drops-por-estado-rojo de hoy. Spec=6 requisitos gate Analista en el .md; review Analista, checker Arquitecto.** **proposed (backlog):** 0229 [WS3 branding], 0230 [WS2 bootstrapper], 0231 [WS4 backend+peones sandbox], 0232 [WS3.5 instalador], 0234 [WS10 runbooks], **0235 [INFRA exec-lease hardening de crons]**. (0233 WS7 e2e diferida: owner Analista, registrar tras cerrar 0228 -- ya cerrada, se puede registrar.)
- **4 SKILLS del Arquitecto** (`.claude/skills/`, TODAS commiteadas): `arquitecto-ledger-ops` (preflight submit_intent), `arquitecto-cron-lifecycle` (destrabar crons: taskkill permitido, Restart Manager `who_locks.py` para zombie huerfano, NO auto-barrido hasta 0235), `mailbox-hygiene`, `arquitecto-monitor-coordina` (loop reactivo + monitor self-filter). USARLAS.
- **COMO COORDINO (semi-auto):** los crons de Codex/Analista corren y COMMITEAN+PUSHEAN al arbol compartido. Armo un `Monitor` sobre origin/main que **ignora mis commits** (los mios llevan `Co-Authored-By: Claude Opus`, los peers no) y despierta con entregas/veredictos. Reacciono: Analista GO->ratifico review_approved (submit_intent) + ACTION done-flip a Codex; NO-GO->remediacion a Codex; entrega Codex->REVIEW al Analista; done->promuevo la siguiente de a una. Ver skill arquitecto-monitor-coordina.
- **PERMISOS crons (fix aplicado 1ff7b72):** settings.json allow tiene `Bash(powershell -NoProfile -File personal/*/*.ps1:*)` + `Bash(taskkill:*)`. Con **modo NO-automatico** puedo relanzar crons; kill siempre. En modo-auto el clasificador anula el allow (deniega spawn-agent).
- **PENDIENTES INMEDIATOS para la nueva sesion:** (1) veredictos RE-DISPARADOS de 0222-rem/0227-rem2 (limpie sus entradas en `analista_mailbox_cron.seen.json` porque el cron los DROPEO -- marca-seen-sin-veredicto; si vuelven a no caer, re-disparar o investigar); (2) terminar HIGIENE mailbox (10/25 hecho, 15 diferidos: batch files `personal/Arquitecto/TX-hyg-{2,3,4}.json`, hacer en VENTANA IDLE de peers, lotes de 5); (3) promover WS3 (0229) al cerrar 0222/0227; (4) promover TASK-0235 (exec-lease) al vaciarse la cola de Codex; (5) considerar registrar WS7 (0233 e2e) ahora que 0228 cerro.
- **LECCIONES CLAVE de esta sesion:**
  - **drift .md/index:** un done-flip/task_status reescribe el `.md` de la tarea Y el TASK_INDEX; al commitear stagea SIEMPRE `tasks/` junto a `state/` -- si no, HEAD queda con `.md`!=`index` y un CLON LIMPIO falla validate (el Analista ancla en clon limpio, no en working tree). Paso con 0223.
  - **Analista dropea reviews:** el cron marca `seen:True` sin producir veredicto -> re-disparar limpiando su entrada en `.protocol-tmp/analista_mailbox_cron/*.seen.json`. (Motiva el lease/heartbeat de 0235.)
  - **higiene mailbox:** NO con peers activos (`Resource deadlock avoided` Errno 36); lotes de 5 en ventana idle; 25-en-una-tx timea a 2min por re-replay. Si timea a mitad: revert `git checkout HEAD -- <state>` + materialize + validate=0.
  - **zombie de cron:** un exec que COMPLETA pero no libera lock/prompt bloquea la cola horas; taskkill //T no agarra al huerfano; usar Restart Manager (`personal/Arquitecto/who_locks.py`) para pinpoint + kill quirurgico. Barrido automatico = NO-GO Analista (posesion!=zombie) hasta el contrato exec-lease (0235).
  - **background-monitor muere con el reinicio del proceso Claude Code** (los crons PowerShell SI sobreviven). Tras reinicio: re-armar el monitor.
  - **Analista = gate adversarial EXCELENTE** (cazo bugs reales en 0225/0227/0222/0228 + mi drift + mis over-claims + el barrido inseguro). Confiar en el, no relitigar sus NO-GO.
- **PENDIENTE DE DISCUSION (operador):** peones en paralelo -> mi lectura: patron valido (peon maker interno + firmante checker), pero el cuello de botella es el REVISOR frontera; para el backlog REQ-ZEUS (diseno) los peones son ROI negativo; usar codegen para lo mecanico; el spike A/B/C (TASK-0231) es experimento medido en SANDBOX, no produccion; el lever real de throughput = frontera-en-paralelo (router D3). El Analista NO alcanzo a revisar esto (queda propuesta).

## >>> ESTADO VIGENTE 2026-06-30 (historico) -- DATASET SELLADO + MEDICION H1-H3 + REQ-ZEUS <<<
- **DATASET SELLADO en N=500.** Tag inmutable `TFM-dataset-N500` -> commit `e3646ae`. Desglose Arquitecto 253 / Codex 195 / Analista 52. Aterrizaje exacto (cierre controlado via higiene). Doble corroboracion (mi conteo + `monitor_dataset_ed25519.py`). Reporte `Area_comun/reports/REPORT-20260630-dataset-tfm-n500-sello.md`. **Operador CONGELO el pre-registro v2.0** (`personal/operador/TFM/FIRMA-OPERADOR-FREEZE-PRE-REGISTRO-v2.md`) -> H1-H3 DES-CEGADAS.
- **MEDICION H1-H3: CERRADA -- 3 HIPOTESIS CONFIRMADAS** (`OPS-MEDICION-H1-H3`, GO del Analista, commit 44c8ead). H1 (det 450/450, FPR 0%, AC2 100%); H2 (dlat med 1.60ms/p95 2.28ms, dstore 0.42KB/ev, dtokens 0% estructural); H3 (acuerdo 500/500, hash clon-limpio dd2fd60e match, 500 ed25519 solo-publicas). Mecanismo real + inyeccion programatica k=50 + reproducibilidad re-verificada (Arquitecto + Analista independiente). Informe final `Area_comun/reports/REPORT-20260630-medicion-H1-H3-auditoria.html` (autocontenido); datos+scripts `personal/Arquitecto/TFM-medicion/`; veredicto `Area_comun/artifacts/ANALISTA-medicion-H1-H3-veredicto.md`; FYI cierre al operador. Limitaciones conservadas (s.8): dtokens estructural; A2/A3 independencia debil; latencia maquina-dependiente.
- **REQ-ZEUS-001 (GOAL-REQ-ZEUS-001, gobernado en HUB; producto en D:\Agentes\Zeus\Zeus-Aegis):** D1-D5 cerradas (NOVA `DECISION-0001..0006`; **D4 hermes-agent=MIT verificado**; 0006 gate doc-only). WS1 **TASK-0226** (remediacion in_progress, doc OK; NO-GO era npm preexistente). **TASK-0227** (npm verde: timeout governance-readonly + boundary **F1/submit_intent** -- F2 desbloqueado post-sello, escalar a decision si es fuga real). **TASK-0225 Arquitecto-cron CONSTRUIDO** (in_review; operador NO lo lanza). Panel lote 0222/0223 ready; 0224 review_approved. Backlog `personal/Arquitecto/REQZEUS-backlog-map.md`; prompt cron `personal/Arquitecto/arquitecto_cron.prompt.txt`.
- **Arquitecto unico escritor:** loop del operador cerrado, cron 0225 NO lanzado -> esta sesion conduce. Si se activa 0225 -> todas las sesiones interactivas de Arquitecto stand-down (un escritor).
- **Skill nuevo:** `.claude/skills/arquitecto-ledger-ops` (checklist preflight). LECCION clave [[arquitecto-gate-own-ledger-ops]]: correr cold-start + GATEAR mis propios mensajes (ASCII + response_owner + claim anidado #self) + COMMITEAR antes de pedir review (el peer valida con clon de HEAD).

## >>> SESION 2026-06-30 (HEAD 825ca6f PUSHED) -- encargo refutacion Engram al Analista + arranque <<<
- **ARRANQUE aplicado** (SESSION_START_PROMPT_20260630): validate/scan_encoding/neutralidad exit 0; 0 claims activos; protocolo PUSHED. **HALLAZGO: Zeus-Aegis estaba 6 commits sin push** (0209/0210/0212/0217/0218 done) -> PUSHEADO (e4cf981..fd26831, 0/0).
- **Operador: foco = avanzar dataset a 500 (trabajo gobernado real); higiene = archivar mailbox + relanzar crons.** Higiene HECHA: 8 msgs resueltos archivados via mailbox_archive (1 tx atomica). LECCION submit_intent --intents: el archivo es `{"intents":[...]}` y cada intent usa clave `"type"` (no "kind"); mailbox_archive exige claim activo cubriendo open+archived paths -> patron ARCHIVE-0164 (scope incluye "Area_comun/state/CLAIMS.json" whole=row-scoped que AUTO-cubre el release CLAIMS.json#claim_id). acquire requiere task_id no-vacio.
- **ENCARGO PRINCIPAL (operador, 2 mensajes): REFUTACION ADVERSARIAL de la propuesta Engram al ANALISTA.** Drafts del Arquitecto a romper: personal/Arquitecto/DRAFT-DECISION-engram-memory-backend.md + PATCH-engram-observation-intent.md (intent `engram_observation`: ledger lleva hash+metadato, cuerpo en Engram fuera de banda, capability OFF). **TASK-0219 (review, ready/Analista; reviewer=Analista, author_under_review=Arquitecto) registrada via submit_intent + GO al Analista (open).** Mandato A-H: A no-interferencia TFM (diferimiento enforzable?), B hash-no-cuerpo I4 (title/topic_key texto libre = PII estructural o disciplinaria? leccion 0040; blacklist burlable?), C single-writer vs 0020 (dos fases ledger/mem_save), D indice reconstruible sin importador, E namespacing map-<id> como autor, F frontera engram!=ledger sombra, G neutralidad, H correccion parche (replay no-op cero-drift, gate ambos caminos single+--intents rollback, fall-through decision intacto). + BRIEFING: verificar Engram contra FUENTE PRIMARIA (README+ARCHITECTURE+TEAM-USAGE+DOCS de Gentleman-Programming/engram); premisa falsa del draft = bloqueante. Independencia: yo soy author_under_review -> NO pre-digerir fuentes por el Analista.
- **CRON ANALISTA LANZADO** (operador GO; el modo-auto deniega powershell via Bash pero el lanzamiento en background paso con el operador presente). El cron tomo GO-0219 en el 1er poll (EXEC 16:55->17:03, ~8min) y libero su claim. [[session-start-request-cron-permission]]
- **VEREDICTO ENTREGADO: NO-GO (HEAD 104f1cc PUSHED).** `Area_comun/artifacts/ANALISTA-TASK-0219-veredicto.md` (Analista commit c572435, autor real Analista). El Analista verifico Engram contra FUENTE PRIMARIA (commit 44faeee) y cazo 2 PREMISAS INEXACTAS del draft (hallazgo: "sin campo de autor en TODO registro" -> memory_relations tiene marked_by_actor; "no documenta merge" -> documentan chunks append-only/no-merge-conflicts). **6 BLOQUEANTES:** B-PII (title sigue texto libre = disciplinario NO estructural; blacklist solo de cuerpo, no allowlist de title/topic_key), C-atomicidad (engram_bridge dos fases sin outbox/ack/reconciliador), D-reconstruccion (no hay importador markdown->engram), E-identidad (map-<id> sin enforcement actor->project server-side), H-parche (falta impl+tests en AMBOS caminos single/--intents+rollback+replay-noop), A-activacion (diferimiento Tier1 disciplinario sin flag default-false + politica no-accesible-por-agentes). Gates clon limpio verdes, guardrail TFM intacto. Recomendacion: solo Tier-0 local/off-ledger; NO promover DECISION ni aplicar PATCH.
- **ARQUITECTO ACEPTA NO-GO** (REVIEW-RESPONSE 104f1cc): drafts quedan `draft`; no promuevo DECISION ni aplico PATCH. TASK-0219 status queda `ready` (orphan cosmetico: cerrar review exige implementer que no tengo; validate exit 0).
- **REPORTE actualizado** (Zeus-Aegis pipeline.html 697bad9): STATUS.updated=hora + STATUS.dataset=412/500 (Arq 195/Codex 177/Analista 40) + bitacora 06-29 (panel pulido, metodologia, Engram NO-GO). FEEDBACK operador: los reportes deben llevar HORA + ESTADO DEL DATASET.
- **2a RONDA EN VUELO (operador GO; HEAD a7fbd11 PUSHED):** el Arquitecto produjo v3 (archivos `-v2`: DRAFT-DECISION-engram-memory-backend-v2.md + PATCH-engram-observation-intent-v2.md) que responde a los 6 bloqueantes NO con "cerrado" sino con una MATRIZ DE ESTADO HONESTA (etiquetas ESTRUCTURAL | ESTRUCTURAL-PENDIENTE-IMPL+TESTS | ESTRUCTURAL-SI-PRECONDICION(<cual>) | DISCIPLINARIO | ABIERTO-DIFERIDO; fix real de B en scope/task_id/supersedes; precondiciones runtime-tier/chain_enabled/actor_auth_enforce/dataset_seal; PATCH = SPEC no implementado). v1 (sin sufijo) intacto = audit trail. **TASK-0220 (review ready/Analista) + GO** registrados. Mandato NUEVO: NO re-litigar lo aceptado ABIERTO/DISCIPLINARIO; ATACAR que las ETIQUETAS sean HONESTAS+COMPLETAS: (1) sobre-afirmacion residual, (2) B-PII hermetico por TODOS los campos a events.jsonl (scope/task_id/supersedes/topic_key/memory_project), (3) precondiciones completas patron-0040, (4) API/config inventada vs fuente runtime/. Verdict por etiqueta HONESTA|AUN-SOBRE-AFIRMA|PRECONDICION-FALTANTE -> GO-PROMOVER-OFF / NO-GO. Round-1 archivado. Cron Analista vivo, tomara el GO en su poll.
- **RONDA 2 (TASK-0220) VEREDICTO: NO-GO pero PROGRESO** (etiquetas casi todas HONESTAS, verificadas vs fuente real; precondiciones confirmadas tier/chain/actor_auth; API no inventada). 3 correcciones minimas: (1) canonicalidad -- los drafts -v2 NO estaban en HEAD (untracked); (2) B-PII AUN-SOBRE-AFIRMA (nit-900123456 pasa slug regex de topic_key/supersedes -> PII corta DISCIPLINARIA, no estructural); (3) citar commit canonico en el GO. LECCION (footgun): el cron Analista se AUTO-DETUVO por el regex stop-order que trata "para" (preposicion) + "Analista" en una linea como orden de parar -> reescribir el GO sin "para" y relanzar. [[analista-cron-stop-order-footgun]].
- **RONDA 3 (TASK-0221) EN VUELO (operador GO: aplicar 3 fixes + ronda 3; B = relabel honesto).** APLICADAS: (1) commitee v1(audit)+v3(-v2) a HEAD canonico **77dfa0f**; (2) relabel fila B "B-PII/cero-prosa" -> "B-cero-prosa-libre (PII corta=DISCIPLINARIO)" + residual explicito; (3) GO-0221 cita 77dfa0f. TASK-0221 registrada + GO (sin footgun "para"). REVIEW-RESPONSE-0220 enviado; round-2 archivado. HEAD 5952acd PUSHED. Mandato ronda 3 acotado: confirmar que las 3 correcciones cierran honestamente -> GO-PROMOVER-OFF / NO-GO. Cron Analista vivo, monitor armado para `ANALISTA-TASK-0221-veredicto.md`.
- **RONDA 3 (TASK-0221) VEREDICTO: GO-PROMOVER-OFF** (3 correcciones CERRADA-HONESTA, 7 etiquetas HONESTA, 5 slips -> NO SLIP, gates clon limpio, guardrail intacto). **DECISION-0071 PROMOVIDA accepted, capability OFF** (con GO del operador): `Area_comun/decisions/DECISION-0071-engram-memory-backend.md` (ASCII-limpio: cambie n/N/sec. por nñÑsec-simbolo; decisions/ exento de neutralidad y NO en ASCII-channel, pero lo deje ASCII por consistencia + memoria simbolo-seccion). Registrada via submit_intent decision-intent (claim PROJECT_STATE+CLAIMS -> decision -> release). Mechanism specified NO merged; PATCH = SPEC hasta merge+tests I1..I10. Tier-0 permitido; Tier-1 NO activado (A/B/E/H pendientes; C/D abierto; PII corta disciplinaria). HEAD edac289 PUSHED. Ciclo Engram cerrado (round 1/2/3 archivado, REVIEW-RESPONSE-0221 handshake). Dataset 439/500 (las 3 rondas + promocion sumaron +27 elegibles).
- **PENDIENTE (operador decide):** las ~10 tareas diferidas ENG-* (ENG-FLAG-WIRING, ENG-OUTBOX/BRIDGE, ENG-ACTORAUTH-ATTEST, ENG-IMPORT, ENG-RECONCILE, ENG-TOPICKEY-PII, ENG-MERGE-SEMANTIC, ENG-REGENESIS-RUNBOOK...) NO se registraron en TASK_INDEX todavia (evitar ruido; todas gated post-dataset/Tier-1). Estan documentadas en la DECISION-0071. Registrar cuando se greenlightee construir Tier-1. TASK-0219/0220/0221 quedan `ready` (orphans cosmeticos de review-task).
- **LECCION:** promover una DECISION = (1) crear archivo canonico en Area_comun/decisions/ desde el draft (ASCII, frontmatter: decision_id + status accepted + deciders + provenance) (2) submit_intent decision-intent {decision_id} con claim PROJECT_STATE+CLAIMS (3) gates (4) commit+push. El decision-intent solo registra decision_id; el contenido vive en el archivo. relacion: [[reports-include-time-and-dataset]] [[analista-cron-stop-order-footgun]].
- **ZEUS-PROTOCOL (b2b2395 PUSHED): doble-click -> modal de detalle completo en Mailbox/Intake/Backlog** (port de TASK-0218 de Zeus-Aegis). zeus-protocol = consola VANILLA JS (public/app.js + styles.css, sin build, node --test, server `node src/server.js` puerto PORT||4173; repo D:/Agentes/Zeus/Zeus-protocol, distinto de Zeus-Aegis). Modal generico nuevo (openDetailModal/ensureDetailModal/closeDetailModal + builders mailboxDetailModel/taskDetailModel/candidateDetailModel) con backdrop+Esc+focus-trap (los modales del wizard de intake NO los tenian). Mailbox/backlog cards = createElement -> dblclick directo; intake cards = innerHTML string -> delegacion en el bind (readCandidateDraft por data-candidate-id). PII via redactRequirementText (mailbox/intake; backlog es data protocolo). Guard: dblclick ignora botones/inputs/labels. node --test 109 (87/22skip/0fail); render headless: backlog 24 cards + mailbox 17 cards -> modal con contenido + Esc cierra; intake 0 candidatos (sin data, cableado identico). NOTA: backlog modal muestra solo campos (TASK_INDEX slim no trae description/AC; el cuerpo completo de la tarea vive en el .md, no cargado -> ampliar server si se quiere). LECCION: zeus-protocol render mas estable que vite-dev (servido as-is, sin hydration).
- **INSTRUMENTACION tokens de la sesion interactiva (protocolo 7c478f8 + producto dc591dc PUSHED):** `scripts/session_token_usage.py` NUEVO suma el uso REAL (message.usage input+output) de los transcripts de Claude Code (`~/.claude/projects/d--Agentes-multi-agent-project-protocol/*.jsonl`) -- el Arquitecto corre interactivo (sin cron) asi que no dejaba run log. Identity/host-neutral (agente + dir por arg). El endpoint /agent-metrics lo fusiona sobre el agente interactivo (env ZEUS_AEGIS_INTERACTIVE_AGENT default Arquitecto + ZEUS_AEGIS_CLAUDE_TRANSCRIPTS default convencional). REAL: **Arquitecto 50.112.152 tokens / 44 sesiones** (antes 0); total 78.5M/202 runs. CACHE (12.4B lecturas) EXCLUIDO por no ser comparable (uso input+output billable). Fast-path por substring '"usage"' -> <1s. 4 gates verdes (build/smoke/f0-test/render). LECCION: el uso de tokens de Claude Code esta en message.usage de los transcripts ~/.claude/projects/<slug>/*.jsonl.
- **PRODUCTO Zeus-Aegis (26c6472 PUSHED): VISTA ESTADISTICAS con COSTO DE TOKENS real (DECISION-0070).** Operador: "construyela, el dashboard debe mostrar el costo de tokens, le aporta valor". Endpoint read-only `/api/governance/agent-metrics` (governance-readonly.ts getGovernanceAgentMetrics) fusiona `scripts/agent_metrics.py` (calidad/tiempo del ledger) + `scripts/agent_token_usage.py` NUEVO (tokens REALES: los crons codex-CLI imprimen "tokens used" + conteo en `.protocol-tmp/*/runs/*.err.log`; mapeo cron->agente DERIVADO en runtime sin literales = neutralidad ok). Vista acordeon 'Estadisticas' en governance.tsx (totales + por agente done/rechazadas/tasa/ciclos/lead/cycle + TOKENS destacados). REAL: total 28.4M tokens/158 runs, Codex 18.576.797 (98 runs), Analista 9.839.907 (60 runs); Arquitecto/Claude/Operador 0 (sin cron, sesion interactiva). Read-only, no contamina. LECCIONES: (a) endpoint nuevo -> anadir a GOVERNANCE_READONLY_ENDPOINTS + actualizar el `toEqual` de governance-readonly.test.ts + la lista de governance-bridge-smoke.mjs (si no, f0/smoke fallan); (b) ruta nueva src/routes/api/*.ts -> el build regenera routeTree.gen.ts, COMMITEARLO; (c) scripts/*.py los escanea neutralidad -> NO hardcodear ids de agente (Analista/Codex/Arquitecto), derivarlos; (d) playwright sin chromium -> channel:'chrome'; acordeon colapsado -> click button[aria-expanded]. PENDIENTE para tokens del Arquitecto/Claude: instrumentar captura (no corre por cron). 4 gates: build/smoke/f0-test/render exit 0.
- **PRODUCTO Zeus-Aegis (f5acd02 PUSHED): boton read-only 'Preparar archivado' en el Mailbox** (`vendor/hermes-2.3.0/src/routes/governance.tsx`): en la fila de filtros, genera el submit_intent mailbox_archive (claim acquire scope open+archived -> mailbox_archive por mensaje OPEN mostrado -> release) en un modal con comando + JSON + Copiar. **El panel NO escribe el ledger** (F2 write-through SIGUE gateado, DECISION-0064; el operador eligio read-only en vez de override). buildArchiveTransaction + ArchivePlanModal (reusa scaffolding del modal 0218). Gates build/smoke/f0-test exit 0 + render headless (Chrome del sistema via channel:'chrome'; playwright NO trae chromium -> usar channel chrome; acordeon Mailbox colapsado por defecto -> click el button[aria-expanded] para expandir antes de buscar el boton). DIAGNOSTICO: el toast 'Failed to fetch usage data' = medidor de tokens de Hermes (usage-meter.tsx) sin gateway = mismo gateway-absent que el chip unknown; cosmetico, se deja.

## >>> RESUME CIERRE 2026-06-29 (HEAD d49376c PUSHED) -- panel pulido+robusto; metodologia atestada+stats+skill; cola vacia <<<
- **Prompt de inicio NUEVO:** `personal/Arquitecto/SESSION_START_PROMPT_20260630.md` (que hacer + como, con gotchas). Detalle completo en `memory/project-state-snapshot.md` (RESUME CIERRE 2026-06-29).
- **PANEL Zeus-Aegis (DONE+PUSHED, HEAD producto fd26831 -- verificar push):** 0209 perf + 0210 UX + 0212 load-resiliente + 0217 backlog (Dashboard==Backlog==171) + 0218 modal detalle por DOBLE-CLICK (Esc/boton "Cerrar (Esc)"). Follow-up menor: redactor PII sobre-redacta fechas ISO como [PHONE-REDACTED] (fail-safe, refinar).
- **METODOLOGIA (DONE):** DECISION-0069 (ceremonia instanciacion ATESTADA: keygen firmante + roster firmantes/workers-keyless + binding LLM + invariante frontera + provenance) + DECISION-0070 (stats agente/peon) ACCEPTED. TASK-0213 ceremonia (Analista refuto V2 binding actor->keyid -> Codex fix guard write-time en submit_intent NO-pineado + V1 keygen; **GUARDRAIL INTACTO, 5 pineados byte-identicos**) DONE. TASK-0214 (`scripts/agent_metrics.py`) DONE. TASK-0216 (`skills/delegate-to-worker.skill.md` off-by-default) DONE.
- **AMBOS CRONS DETENIDOS. Cola activa VACIA** (las ready son review-tasks huerfanas + proposed largo-plazo).
- **PENDIENTE OPERADOR (no ejecutar sin GO):** vistas panel stats (consume agent_metrics) + accion instanciar-proyecto; V2 verify-time POST-ventana 500; fix redactor fechas; PII postura por instancia; activar Arquitecto-cron (ScheduleWakeup no disparo fiable de noche).
- **LECCIONES:** render headless vite-dev FLAKY [[checker-render-headless-vite-dev-flaky]]; con cron vivo no dejar GO en disco antes de registrar la tarea (lo consume/marca seen; fix=editar msg refresca hash); mailbox_archive en lotes de ~10 (tx grande -> timeout+drift; restaurar events.jsonl no-committeado); clean-clone sin dist residual.
...
- **CORRECCION RUMBO (operador 2026-06-28): NOVA-Budget NO se arranca todavia (STAND-BY); el FORK Zeus-Aegis necesita AJUSTES.** Al reanudar: preguntar QUE ajustes (candidatos: dev-script Windows-safe via cross-env [bug real visto], reusar conductor Hermes para lanzar agentes, retirar bloat 3D/Electron, 24 fallos upstream, F3 chat/PWA, versionado). NO ingerir el diseno de DB ni redactar DECISION de NOVA hasta nuevo GO. Detalle en SESSION_START_PROMPT_20260628.md.
