...
- **DIRECTIVA ACTIVA: medicion H1-H3** (`OPS-MEDICION-H1-H3-20260630`) sobre el tag, segun `personal/operador/TFM/PLAN-EJECUCION-MEDICION-H1-H3.md` (Fases 0-5). Umbrales s.5 (`PRE-REGISTRO-H1-H3-v2.md`): H1 deteccion=100% A1/A2/A3 & FPR=0% & salud AC2>=99%; H2 dlat med<=50ms p95<=200ms, dstore<=4KB/ev, dtokens<=5%; H3 verificador externo=100% & hash canonico clon-limpio. Reglas: medir SOLO sobre el tag, inyeccion PROGRAMATICA reproducible, H3 clon sin secretos, reportar fallos sin maquillaje. Entregable: **informe HTML auditoria autocontenido** (A4 serif, sin JS) + gate adversarial Analista + cierre gobernado.
