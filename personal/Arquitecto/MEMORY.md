...
- **BLOQUEADO en relanzar crons:** el modo-auto DENIEGA powershell via Bash (clasificador). **Necesito al operador en modo DEFAULT para aprobar el prompt y lanzar `personal/Analista/analista_mailbox_cron.ps1` (+ Codex).** Sin el cron del Analista vivo, el GO-TASK-0219 NO se procesa. [[session-start-request-cron-permission]]
- **PROXIMO:** operador en DEFAULT -> lanzo crons -> Analista toma GO-0219, verifica fuentes, entrega `Area_comun/artifacts/ANALISTA-TASK-0219-veredicto.md` firmado -> yo recibo veredicto (GO/NO-GO/condiciones) -> si CAMBIO-REQUERIDO corrijo drafts o emito SDD a Codex; si GO promuevo DECISION. NO promover la DECISION ni aplicar el parche sin el veredicto.
