...
- **SOLO HUB, SIN PRODUCTO.** Detalle completo en `personal/Analista/MEMORY-TASK-0368-r1-20260814.md`
  (ancla `89af4fdb`, veredicto commit `9a7ce58b`) y `personal/Analista/MEMORY-TASK-0368-r2-20260814.md`
  (ancla `31867185`, veredicto commit `de2d6c55`). Artefactos:
  `Area_comun/artifacts/Analista-TASK-0368-r1-vigencia-por-lista-atestada-verdict.md` y
  `Area_comun/artifacts/Analista-TASK-0368-r2-current-with-warning-verdict.md`.
- **r2: las SEIS puertas del `verification_cmd` en exit 0** y aun asi CHANGE-REQUIRED. Cerrados B1
  (10 fronteras, 9 discriminan), B2, B4 (9/9 miembros pinados) y la mitad ruidosa de B3. Abiertos:
  la grafia NO registrada la descarta `validate_metadata` aguas arriba y cae en la rama de AUSENCIA
  -> `current_with_warning` -> vigente + exit 0 (13/13 grafias; e2e real: decision vigente 109);
  B5 sigue indefenso (el mutante de produccion que ignora `superseded_by` sobrevive las 73 pruebas
  porque su frontera compara dos poblaciones distintas); `missing_status` es un literal pinado que
  el motor nunca desreferencia.
- **Las cuatro lecciones que me llevo:** (1) preguntar que tiene que ser cierto de la entrada para
  LLEGAR a la rama nueva -- un filtro aguas arriba la vuelve inalcanzable; (2) una remediacion que
  estrecha sin cambiar la clase se mide con DOS cardinalidades, y decir si la que sigue pasando es
  finita; (3) un `assertNotEqual` entre conjuntos de POBLACIONES distintas es tautologia -- mutar
  produccion y correr la suite ENTERA es lo unico que lo delata; (4) un campo de configuracion que
  nadie desreferencia es decoracion aunque este pinado.
- Escalada: el Arquitecto habia fijado dos vueltas como techo y esta era la segunda; se lo devolvi
  como decision suya (escalar al operador vs remediacion acotada de tres arreglos mecanicos), con
  los tres criterios de aceptacion escritos POR CONDUCTA.
