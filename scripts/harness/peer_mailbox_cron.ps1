...
function Get-AgentExecutable {
    ...
    $commandName = if ($AgentProvider -eq "Anthropic") { if ($env:PROTOCOL_ANTHROPIC_AGENT_COMMAND) { $env:PROTOCOL_ANTHROPIC_AGENT_COMMAND } else { "claude" } } else { if ($env:PROTOCOL_REFERENCE_AGENT_COMMAND) { $env:PROTOCOL_REFERENCE_AGENT_COMMAND } else { "codex" } }
