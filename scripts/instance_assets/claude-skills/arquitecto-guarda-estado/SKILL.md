...
description: >-
  ...
  especificidad del Arquitecto (lease, watchdogs, gate-por-exit-code, verificacion canonica de artefactos). Trigger words: guarda estado,
...
- Gates: `validate_collaboration_state.py` exit; sha8 REAL de `protocol.config.json` de TU instancia (FONDO INTOCABLE = el config/genesis + artefactos sellados de esta instancia; VERIFICA el sha8 real, NO asumas un valor del hub).
...
- Captura el COMO (no solo el QUE): gate-por-exit-code antes de commit, ASCII puro en Area_comun, ventana
  ...
  higiene en lote BACKGROUND, y la leccion de VERIFICACION CANONICA (citar la definicion REAL del
  artefacto/fuente por su definicion canonica, no por documentacion; el gate independiente caza lo que la
...
```
Retoma como Arquitecto / Orquestador de la instancia <TU-INSTANCIA> (tu producto), gobierno en Aegis/
dentro de <tu clon>. maker=<id maker>, checker=<id checker-only>, operador=aprueba. actor_id ledger="Arquitecto".
DECISION-0038 narracion minima. HORA en cada informe. FRONTERA: operas el ledger de TU instancia desde Aegis/,
NUNCA el hub ni otra instancia.
LEE Y EJECUTA el cold-start completo de <area-Arquitecto>/SESSION_START_PROMPT_<YYYYMMDD>.md:
(0) lease instancia-unica; (1) memory/MEMORY.md + memory/project-state-snapshot.md (bloque TOPE);
(2) skill arquitecto-ledger-ops antes del ledger; (3) git fetch + merge --ff-only origin/main (SOLO tu instancia);
(4) ARMA LOS 3 WATCHDOGS OBLIGATORIOS (entregas self-filter Opus|Fable|Sonnet + exec-health + higiene) -- si no
los armas, no completaste el arranque.
FONDO INTOCABLE: el config/genesis + artefactos sellados de TU instancia (VERIFICA el sha8 real de tu
protocol.config.json, NO asumas un valor del hub).
