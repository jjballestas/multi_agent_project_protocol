...
$cases = @(
    @{ Name = "valid_enforced"; Exit = 0 },
    @{ Name = "missing_required_fields"; Exit = 1 },
    @{ Name = "presdd_exempt"; Exit = 0 },
    @{ Name = "lightweight_warnings"; Exit = 0 }
)
...
if (-not $pythonRuntime -and -not $powershellRuntime) {
    Write-Host "FAIL: no Python or PowerShell runtime resolved; cannot verify SDD validation cases."
    $failed = $true
}
...
Write-Host "OK: SDD validation cases passed."
