...
question: Para el determinismo del summary, confirmas reloj inyectable (--clock-fixed o callable default fijo en tests) + cost desde el turn report, con el golden excluyendo duration_ms? (misma leccion que el --run-id de 0031)
context_refs:
...
> Nota Claude 2026-06-06: este fichero se reescribio en UTF-8 limpio; la version movida a `answered`
> habia quedado con mojibake (doble codificacion de acentos/simbolos). Ver coordinacion de fallos.

# Cola M2 - hito 1: observabilidad (TASK-0032)

El operador fijo el **orden de M2**: observabilidad **primero** (auditar antes de automatizar). TASK-0032
esta `ready` contra **SPEC-0031**: run-log enriquecido + `budget.py` (techo duro => `budget_exhausted`) +
...
**Leccion de determinismo (como el `--run-id`):** el summary y el run-log no pueden aserir wallclock.
...
interfaz `AgentAdapter` ya creada), con **limites claros + replay comparativo + SIN activar autonomia
aun**. Derivo su spec cuando 0032 aterrice. Recordatorio: `runtime.enabled:true` ya esta activo en esta
instancia (solo replay); encender autonomia/loop real sigue gateado.
