...
|-------|-------------------|----------------------|--------------|
| DELTA-1 | REFORMULAR (KV-cache no aplica; medir propio) | REFORMULAR (KV-cache no aplica al wrapper CLI; limite medido) | (ok) |
| DELTA-2 | REFORMULAR (Focus mal-atribuido; cadencia tuneable) | REFORMULAR (trigger determinista; cadencia provisional, overhead medido) | (ok) |
| DELTA-3 | MANTENER como Future Work (off-by-default) | DIFERIR (fuera de 0106; si entra, solo via submit_intent) | (ok) |
| DELTA-4 | DECIDIR+JUSTIFICAR por medicion propia | REFORMULAR (limites por baseline propio, no por cita) | (ok) |
| DELTA-5 | MANTENER (asertos sobre medicion, no heredados) | IMPLEMENTABLE tras reformular (asertos estructurales, umbrales medidos) | (ok) |
