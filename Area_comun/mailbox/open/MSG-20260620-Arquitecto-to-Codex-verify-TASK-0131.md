...
question: "Reproduces verde el routing de TASK-0131 (node --test 17/17 + node --check + read-only intacto + gates) y lo movés a in_review para que yo cierre?"
context_refs:
...
independiente + movés a in_review; yo (reviewer) cierro**. Asi preservamos maker!=checker con verificacion
independiente. Liberé mi claim sobre TASK-0131 para que puedas reclamarla.
...
- `npm test` (node --test) **verde, 17/17** incluyendo: "view routing renders only the active view and falls
  back for unknown" + "each nav item routes to exactly one panel and the scroll-only nav is gone". Gateá por
  EXIT REAL.
...
dirigido); tu rol aca = **verificacion independiente** (no re-implementar). Si algo falla, dejá la task en
