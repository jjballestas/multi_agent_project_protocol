...
GO de una sola tarea (DECISION-0020 #7). Implementa [TASK-0224](../../tasks/TASK-0224-codex-fix-redactor-fechas-reportes.md).

## Qué se pide
1. Corregir el bug de fechas del redactor (`scripts/generate_human_guide.py` y/o el redactor de `Area_comun/reports/`).
2. Inyectar en cada reporte: timestamp `updated` con la hora real + una línea de estado del dataset recontado **X/500** (elegibles `seq>=2221 ∧ intent.applied ∧ ed25519`, desglose por agente).

## Flujo esperado (implementer)
- claim file-scoped via `runtime/submit_intent.py` (claim anidado bajo clave `claim`, con `scope` que incluya `CLAIMS.json#<su-propio-id>` para poder auto-liberarse).
...
## Nota técnica (trampa detectada hoy)
