...
question: Para el determinismo del summary, Â¿confirmas reloj inyectable (--clock-fixed o callable default fijo en tests) + cost desde el turn report, con el golden excluyendo duration_ms? (misma lecciÃ³n que el --run-id de 0031)
context_refs:
...
# Cola M2 â€” hito 1: observabilidad (TASK-0032)

El operador fijÃ³ el **orden de M2**: observabilidad **primero** (auditar antes de automatizar). TASK-0032
estÃ¡ `ready` contra **SPEC-0031**: run-log enriquecido + `budget.py` (techo duro â‡’ `budget_exhausted`) +
`metrics.py` (`summarize` post-hoc â‡’ `RUN-<id>.summary.json`) + `--budget-tokens`. Aditivo y
off-by-default; el motor apply/gate/vcs (0030) y el loop (0031) **no se reabren**.

**LecciÃ³n de determinismo (como el `--run-id`):** el summary y el run-log no pueden aserir wallclock.
...
**Siguiente hito (no ahora â€” para que tengas el mapa):** **adapter LLM real** (Claude/Codex tras la
interfaz `AgentAdapter` ya creada), con **lÃ­mites claros + replay comparativo + SIN activar autonomÃ­a
aÃºn**. Derivo su spec cuando 0032 aterrice. Recordatorio: `runtime.enabled:true` ya estÃ¡ activo en esta
instancia (solo replay); encender autonomÃ­a/loop real sigue gateado.
