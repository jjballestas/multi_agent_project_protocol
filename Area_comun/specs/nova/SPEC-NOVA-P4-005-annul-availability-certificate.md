...
- **PRECONDICION: el proc de hardening YA EXISTE y esta VERIFICADO por el DBA** (10/10 pruebas en
  `DbsFinanciero_SANDBOX`: existencia, guarda bloqueante hijos-vivos THROW 50293, camino feliz con cuadre
  CDP, idempotencia, rollback forzado, `SESSION_CONTEXT` faltante). GRANT EXECUTE ya concedido a
  `budget_sandbox_verifier` (grant surface 107). **PENDIENTE ANTES DE CONSTRUIR:** `GRANT VIEW DEFINITION`
  sobre `Annul_Availability_Certificate` + sus tablas de reverso (`Availability_Certificate_Reversal`,
  `Availability_Certificate_Reversal_Line`) + `GRANT SELECT` sobre las tablas BASE que el proc consulte
  internamente (leccion de TASK-0254/P4.2: SQL 229 por `SELECT` denegado sobre una tabla de cabecera NO
  anticipada) -- pedir el preflight ampliado ANTES del GO, no esperar a que F-NOVA-01 falle en vivo.
...
- **F-NOVA-01 (re-verificacion, CRITICA -- precedente F-0246-02, P4.1, P4.2):** el maker RE-VERIFICA el set
  EXACTO de THROW del `Annul_Availability_Certificate` DESPLEGADO via `OBJECT_DEFINITION` (ya tiene VIEW
  DEFINITION una vez concedido). Conocidos por el DBA (validacion 10/10, no exhaustivos de todo el codigo):
  **THROW 50293** (guarda bloqueante: intentar anular un CDP con RP activo encima) y un THROW de
  `SESSION_CONTEXT('tenant_id')` faltante (familia 50100, RE-VERIFICAR el numero exacto -- no asumir que
  es el mismo 50100 usado en otros procs sin confirmar). NINGUN codigo se hornea en un criterio falsable
  sin confirmarlo en la definicion desplegada.
...
- Calidad: falsable. Bien: "anular un CDP que tiene un RP activo encima devuelve ProblemDetails del THROW
  50293 (guarda bloqueante) y NO libera saldo ni cambia el estado del CDP".
...
- **Guarda bloqueante (RN-08):** el proc rechaza la anulacion si el CDP tiene compromisos (RP) activos
  encima -> THROW 50293. La aplicacion NO debe permitir ni siquiera intentar la llamada sin que el
  usuario vea la previsualizacion del estado (RP activos) primero, pero la guarda AUTORITATIVA vive en
  el proc, no en C#.
...
- **THROW (conocidos por la validacion del DBA, RE-VERIFICAR set EXACTO y completo via OBJECT_DEFINITION):**
  **50293** (guarda bloqueante: CDP con RP activo, no anula); THROW de `SESSION_CONTEXT` faltante (familia
  50100, RE-VERIFICAR numero exacto para este proc especificamente); THROW de CDP no-existe / ya-anulado
  (idempotencia) -- el maker enumera el set completo real, no solo estos 2 conocidos.
...
- Propias:
  ...
  - (b) **GUARDA BLOQUEANTE (RN-08):** el proc, NO la aplicacion, decide si hay RP activos que bloqueen la
    ...
    es del proc (THROW 50293, RE-VERIFICAR).
...
> maker CONFIRMA el set completo real (no solo 50293 y el de tenant) antes de fijar los criterios finales.
...
2. **Dado** un CDP CON un compromiso (RP) activo encima, **entonces** ProblemDetails del THROW 50293
   (guarda bloqueante) y el CDP/saldo NO cambian.
...
- **Integracion vs DbsFinanciero_SANDBOX (EXECUTE via GRANT, ya concedido):** criterio 1 (happy, delta de
  saldo real), un caso por THROW ALCANZABLE (50293 guarda, tenant faltante, no-existe, idempotencia)
...
|---|---|---|
| Asumir el set de THROW documentado (solo 50293 + tenant) sin verificar el resto | Criterio falso, THROW no alcanzable no detectado (F-0246-02) | F-NOVA-01: RE-VERIFICAR el set COMPLETO contra OBJECT_DEFINITION, no solo los 2 conocidos |
