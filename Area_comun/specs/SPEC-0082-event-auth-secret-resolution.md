...
# SPEC-0082 - Resolucion del secreto HMAC de event_auth fuera del repo
...
**Thread explicito de `root` (ajuste Codex feasibility, pasada 2026-06-19).** Como `secret_file` es
RELATIVO al root, `signing_secret`/`verify_event_auth` deben recibir `root` EXPLICITO y propagarlo por
TODOS los llamadores que hoy pasan solo `config`: `protocol_replay.replay_events`,
`rebuild_snapshot`, `EventWriter.state()` y los validadores. NO depender del `cwd`. Sin `root`, la
resolucion de `secret_file` en replay/validacion seria no-determinista segun el directorio de invocacion.
(El `secret_env` no depende de root; el literal tampoco.)
...
- **AC4 - Sin secreto literal commiteado (gate DEDICADO).** Un **check dedicado** (en el validador o un
  script llamado por CI) RECHAZA un `secret` literal para cualquier actor de
  `agent_registry`/`event_auth.keys` en el config commiteado; permite `secret_file`/`secret_env` y permite
  `secret` literal solo en fixtures bajo `examples/`. **NO se delega en `scan_encoding` ni
  `scan_domain_neutrality`** (ajuste Codex feasibility): `scan_encoding` no detecta secretos y
  `scan_domain_neutrality` no inspecciona la estructura de `event_auth.keys`; por eso el gate AC4 es un
  check propio, no un efecto colateral de los scans existentes.
