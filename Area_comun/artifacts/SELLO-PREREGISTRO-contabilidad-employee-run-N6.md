# SELLO PRE-REGISTRO N=6 -- Replica employee-run de transferibilidad (Contabilidad, instancia Aegis)

> **ARTEFACTO SELLADO (congelado).** Copia congelada byte-faithful del DRAFT del Asesor
> `personal/asesor/DRAFT-PREREGISTRO-contabilidad-employee-run.md` (preparado 2026-07-12, seal-ready en el
> commit del hub `23b8fe5`), sellada por el Arquitecto el **2026-07-13 (~00:35 local UTC+2 / 2026-07-12T22:35Z)**
> bajo GO del operador (`Area_comun/mailbox/... Operador-to-Arquitecto` GO sello pre-registro N=6 + 6 unidades
> medidas). Registro formal atestado = **DECISION-0094** (guarda el `sha256` de ESTE archivo -- valor que no puede
> auto-referenciarse dentro del propio archivo, patron DECISION-0091 -- + su anclaje via intent `decision` del hub,
> cadena #4). Opcion A confirmada por el operador (pre-registro propio, no exploratoria); seccion hermana del Sello
> Etapa 2. Base: PREP-INSTRUMENTACION-MEDICION-contabilidad-aegis.md (Arquitecto) + SPEC-NOVA-F3.3 +
> DECISION-0088/0093 + delimitacion sellada de Nova-Budget ("veredicto de compra -> replica employee-run
> PRE-REGISTRADA"). NO toca el estudio Nova-Budget congelado ni el config pineado del hub (1.14.0, 2E35F26E).
>
> **REGLA DURA POST-SELLO (s.11 paso 6):** nada de s.1-s.7 (hipotesis, poblacion/muestra N=6, metricas, criterio de
> exito/refutacion) se cambia mirando datos; un cambio exigiria una enmienda FECHADA y justificada, visible en el
> ledger. **Pre-datacion:** este sello se ancla ANTES de construir/medir cualquiera de las 6 unidades (ninguna existe
> aun; build-open post-30-jul). **Precondicion externa NO-hueco:** el cableado de la instrumentacion F3.3 en Aegis
> (s.10) es prerequisito de la 1a unidad MEDIDA, NO del sello del DISENO; el diseno esta completo y se congela aqui.
