...
- Mismo validador en clon limpio de `cae10ad` sin secretos -> exit 0.
- `python scripts/scan_domain_neutrality.py` vivo -> exit 0.
- `python scripts/scan_encoding.py` vivo -> exit 0.
- `protocol_state_drift(Path('.'))` -> `has_drift=false`, `up_to_seq=4693`, exit 0.
- `protocol.config.json` worktree vs blob de `cae10ad` -> byte-identica; SHA-256 `2E35F26E06DE4D0A7E5278BABB2107A9BBE6441C78B99A1886A613070B1EB354`.
...
gates: docs-only; validate vivo/secretless exit 0; domain exit 0; encoding exit 0; drift false up_to_seq 4693; config #4 byte-identica.
