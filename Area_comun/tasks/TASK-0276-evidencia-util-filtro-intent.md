...
intake:
  ...
  acceptance:
    - "Solo cuenta como evidencia de trabajo util un evento propio con applied true y cuyo payload.intent_type este en {task_status, task_upsert, decision}; payload.commit es metadata comun y no evidencia trabajo por si sola, por lo que el par de puro claim acquire/release y los exception.recorded dejan de confirmar incluso si llevan commit."
