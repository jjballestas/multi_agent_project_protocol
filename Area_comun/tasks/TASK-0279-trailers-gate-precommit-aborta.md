...
intake:
  ...
  acceptance:
    - "CORRECCION DE HOOK (autorizada por el Arquitecto 2026-07-22, sobre evidencia del maker): el gate ABORTANTE de trailers vive en .githooks/commit-msg, NO en pre-commit -- pre-commit corre antes de finalizar el mensaje y .git/COMMIT_EDITMSG conserva el mensaje ANTERIOR, produciendo rechazos falsos. commit-msg recibe el mensaje finalizado y su exit distinto de cero aborta el commit antes de crearlo. pre-commit se conserva para el chequeo del snapshot staged; el export born-operational instala AMBOS."
    - "El hook de commit-msg rechaza el commit cuando el mensaje incumple el contrato de trailers, con un mensaje que dice exactamente que falta y como se escribe correctamente."
